...
const Contractors = {
    ...
    isApprovalActive(record, includeExpired = false) {
        ...
        if (!this.isEntityEnabled(record)) return false;
    ...
    renderApprovedEntitiesTable(records, isAdmin = false) {
        ...
        const rowsHtml = records.map((record) => {
            ...
            const activeBadge = isEnabled ? '' : '<span class="badge badge-danger ml-2">غير نشط</span>';
    ...
    buildContractorAnalyticsMatchers(contractor, contractorIdParam) {
        ...
        return {
            normalize: (value) => String(value == null ? '' : value).trim().toLowerCase(),
            idsSet: new Set(),
            matchesContractor() { return false; },
            violationBelongsToContractor() { return false; },
            evaluationBelongsToContractor() { return false; },
            contractorName: String(contractor?.name || contractor?.companyName || '').trim(),
            hasAnyRecordIds() { return false; },
            matchesNameValue() { return false; },
            matchFieldsByName() { return false; }
        };
    },
