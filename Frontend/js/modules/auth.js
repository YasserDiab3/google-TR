...
window.Auth = {
    ...
    /**
     * تسجيل الدخول عبر Supabase Auth فقط (بريد + كلمة المرور).
     * عطّل بـ window.__HSE_SUPABASE_AUTH_EMAIL_PASSWORD__ = false للعودة لجدول Users محلياً.
     */
    shouldUseSupabaseEmailPasswordAuth() {
        try {
            if (typeof window !== 'undefined' && window.__HSE_SUPABASE_AUTH_EMAIL_PASSWORD__ === false) {
                return false;
            }
            if (!window.__HSE_SUPABASE_ANON_KEY__) return false;
            if (!window.__HSE_RPC_URL__ && !window.__HSE_SUPABASE_URL__) return false;
    ...
    async loginViaSupabaseEmailPassword(email, password, remember) {
        const inputPasswordRaw = (password || '').trim();
        if (typeof HseSupabasePasswordAuth === 'undefined' || typeof HseSupabasePasswordAuth.signInWithPassword !== 'function') {
            Notification.error('وحدة Supabase Auth غير محمّلة.');
            return { success: false, message: 'وحدة Supabase Auth غير محمّلة' };
        }

        let sessionData;
        try {
            sessionData = await HseSupabasePasswordAuth.signInWithPassword(email, password);
        } catch (e) {
            const raw = e?.message || String(e);
            try {
                await Utils.RateLimiter.recordFailedAttempt(email);
            } catch (rl) {
                Notification.error(rl.message);
                return { success: false, message: rl.message };
            }
            const friendly = /invalid login credentials|invalid grant|email not confirmed|invalid credential/i.test(raw)
        ...
        if (canSyncUsers && typeof GoogleIntegration.syncUsers === 'function') {
            try {
                await Promise.race([
                ...
                Utils.safeWarn('⚠️ تعذر مزامنة Users بعد Supabase Auth:', syncErr);
            }
        }
        ...
        if (typeof extractedName === 'object' && extractedName !== null) {
            extractedName = extractedName.value ? String(extractedName.value).trim() : String(foundUser.email || '').split('@')[0];
        ...
        const user = {
            name: extractedName || email,
            password: '***',
            passwordHash: '***',
            role: foundUser.role || 'user',
            department: foundUser.department || '',
            factory: foundUser.factory || foundUser.factoryId || '',
            factoryId: foundUser.factoryId || '',
            factoryName: foundUser.factoryName || '',
            subLocation: foundUser.subLocation || '',
            subLocationId: foundUser.subLocationId || '',
            subLocationName: foundUser.subLocationName || '',
        ...
        if (typeof window.__hseRunCompleteLoginFlow !== 'function') {
            Utils.safeError('❌ auth-complete-login-flow غير محمّل');
            Notification.error('خطأ في تهيئة المصادقة.');
            HseSupabasePasswordAuth.clearSession();
            return { success: false, message: 'auth-complete-login-flow مفقود' };
        }

        return await window.__hseRunCompleteLoginFlow(this, {
            email,
            password: inputPasswordRaw,
            remember,
            user,
            foundUser,
            hashedStored,
    ...
    async login(email, password, remember = false) {
        ...
        const usersPrecheck = Array.isArray(AppState.appData.users) ? AppState.appData.users : [];
        const skipSupabaseForBootstrap = usersPrecheck.length === 0 &&
            typeof this.isBootstrapEmail === 'function' && this.isBootstrapEmail(email) &&
            typeof this.isBootstrapDisabled === 'function' && !this.isBootstrapDisabled();

        if (this.shouldUseSupabaseEmailPasswordAuth() && !skipSupabaseForBootstrap) {
            return await this.loginViaSupabaseEmailPassword(email, password, remember);
        }
        ...
        if (typeof window.__hseRunCompleteLoginFlow !== 'function') {
            Utils.safeError('❌ auth-complete-login-flow غير محمّل');
            Notification.error('خطأ في تهيئة المصادقة (ملف auth-complete-login-flow). أعد تحديث الصفحة.');
            return { success: false, message: 'auth-complete-login-flow مفقود' };
        }
    ...
    logout() {
        ...
        try {
            const sbAuth = typeof HseSupabasePasswordAuth !== 'undefined' ? HseSupabasePasswordAuth : null;
            if (sbAuth && typeof sbAuth.signOutRemote === 'function') {
                Promise.resolve(sbAuth.signOutRemote()).finally(() => {
                    try {
                        if (typeof sbAuth.clearSession === 'function') sbAuth.clearSession();
                    } catch (e) { /* ignore */ }
                });
            } else if (sbAuth && typeof sbAuth.clearSession === 'function') {
                sbAuth.clearSession();
            }
        } catch (e) { /* ignore */ }
